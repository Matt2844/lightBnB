...
const getAllProperties = function(options, limit = 10) {
  ...
  if (options.owner_id) {
    ...
    queryString += `AND WHERE owner_id = '${queryParams.owner_id}`;
  }
  if (options.minimum_price_per_night, options.maximum_price_per_night) {
    queryParams.push(`%${options.minimum_price_per_night}%`, `%${options.maximum_price_per_night}%`);
    queryString += `AND WHERE cost_per_night = '${queryParams.minimum_price_per_night}' AND ${queryParams.options.maximum_price_per_night}`;
  }
  if (options.minimum_rating) {
    queryParams.push(`%${options.minimum_rating}%`);
    queryString += `AND WHERE rating > ${queryString.minimum_rating}`;
  }
...
const addProperty = function(property) {
  ...
  return pool.query(query, [owner_id, title, description, thumbnail_photo_url, cover_photo_url, cost_per_night, street, city, province, post_code, country, parking_spaces, number_of_bathrooms, number_of_bedrooms])
    .then(res => res.rows[0])
    .catch(err => console.log('error:', err));
}
